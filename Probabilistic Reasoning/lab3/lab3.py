...
def computePrior(labels, W=None):
    ...
    total_weights_sum = np.sum(W)
    for class_index, class_label in enumerate(classes):
        idx = np.where(labels == class_label)[0]
        class_total_weights = np.sum(W[idx])
        
        prior[class_index] = class_total_weights / total_weights_sum
    # ==========================
...
def classifyBayes(X, prior, mu, sigma):
    ...
    for classe in range(Nclasses):
        ...
        for x_index in range(Npts):
                first_term = -0.5 * np.log(np.linalg.det(sigma[classe]))
...
#mu,sigma = old_mlParams(X,labels)
N = X.shape[0]
